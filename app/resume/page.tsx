    ...
    <main className="flex min-h-screen flex-col items-center w-full pt-32">
      <section className="w-full py-12 px-6 max-w-4xl mx-auto flex-1 flex flex-col justify-center">
        <div className="flex items-center justify-between mb-16">
          ...
          <a href="#" className="px-6 py-3 bg-emerald-400/10 text-emerald-400 font-semibold rounded-full hover:bg-emerald-400/20 transition-colors border border-emerald-400/20 flex items-center gap-2">
            ...
            <div className="space-y-8 pl-4 border-l-2 border-white/10">
              {experience.map((exp, index) => (
                <div key={index} className="relative">
                  <div className="absolute w-3 h-3 bg-emerald-400 rounded-full -left-[23px] top-2" />
                  <div className="mb-1 flex flex-col sm:flex-row sm:items-center justify-between">
                    <h4 className="text-xl font-bold">{exp.role}</h4>
                    <span className="text-emerald-400/80 text-sm font-medium">{exp.duration}</span>
                  </div>
              ...
              {education.map((edu, index) => (
                <div key={index} className="relative">
                  ...
                  <div className="mb-1 flex flex-col sm:flex-row sm:items-center justify-between">
                    ...
                    <span className="text-emerald-400/80 text-sm font-medium">{edu.duration}</span>
                  </div>
                  <div className="text-white/60 font-medium mb-3">{edu.university}</div>
